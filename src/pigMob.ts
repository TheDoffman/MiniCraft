...
import { getPigMaterialBundle } from './pigTextures';
...
export function updatePig(pig, world, dt) {
  ...
  pig.fleeT = (pig.fleeT ?? 0) - dt;
  if (pig.fleeT > 0) {
    const fleeSpeed = 2.2;
    pig.targetVx = (pig.fleeDirX ?? 0) * fleeSpeed;
    pig.targetVz = (pig.fleeDirZ ?? 0) * fleeSpeed;
    pig.wanderT = 1 + Math.random() * 2;
  } else {
    pig.wanderT -= dt;
    if (pig.wanderT <= 0) {
  ...
  if (inWater) {
    const surf = getColumnWaterSurfaceY(world, pig.x, pig.z);
    if (surf !== null) {
  ...
  if (!stillInWater && (onGround || landed)) {
    ...
    const ty = world.topSupportingSolidY(ix, iz, pig.y);
    if (ty >= 0) {
...
export function damageNearestPig(_scene, pigs, damage, _atlasTex, _drops, pig, knockDx, knockDz) {
  ...
  if (pig.hp > 0) {
    if (hlen > 1e-5) {
      pig.fleeDirX = kdx / hlen;
      pig.fleeDirZ = kdz / hlen;
    } else {
      const fa = Math.random() * Math.PI * 2;
      pig.fleeDirX = Math.cos(fa);
      pig.fleeDirZ = Math.sin(fa);
    }
    pig.fleeT = 2.5 + Math.random() * 1.5;
    return true;
  }
  pig.state = 'dying';
  pig.deathT = 0;
