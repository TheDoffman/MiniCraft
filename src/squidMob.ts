...
import { getSquidMaterialBundle } from './squidTextures';
 ...
 * @param {number} index
 */
export function finalizeDeadSquid(scene, squids, index, atlasTex, drops) {
  const s = squids[index];
  const ink = createGroundDrop(
    s.x,
    s.y + 0.2,
    s.z,
    BlockId.INK_SAC,
