...
/** When not swimming up/down: gentle downward acceleration (blocks/s²). */
const WATER_IDLE_SINK_GRAVITY = -3.6;
/** Max downward speed while idle in water (vy is clamped so it does not sink faster than ~2 blocks/s). */
const WATER_IDLE_SINK_CAP_VY = -2.05;
...
/** Lava: slower than water, stronger downward pull, dimmer swim-up. */
const LAVA_MOVE = 1.35;
const LAVA_DRAG = 8.5;
const LAVA_GRAVITY = -18;
const LAVA_BUOY = 6.2;
const LAVA_VY_MAX = 3.2;
const LAVA_VY_MIN = -7.5;
...
export class Player {
  ...
  _maxFallSpeed: number;
  /** Damage to apply this frame after landing; read by game loop. */
  fallDamageThisFrame: number;
  /** Creative-mode flight: true when the player is flying. */
  flying: boolean;
  /** Scratch for {@link Player#aabb} — do not mutate returned arrays' contents across frames carelessly. */
  _aabMin: [number, number, number];
  _aabMax: [number, number, number];
  ...
  aabb() {
    this._aabMin[0] = this.x - this.halfW;
    this._aabMin[1] = this.y;
    this._aabMin[2] = this.z - this.halfW;
    this._aabMax[0] = this.x + this.halfW;
    this._aabMax[1] = this.y + this.height;
    this._aabMax[2] = this.z + this.halfW;
    return { min: this._aabMin, max: this._aabMax };
  }
   ...
   * @param {{ forward: boolean, back: boolean, left: boolean, right: boolean, jump: boolean, swimDown?: boolean, sprint?: boolean }} input
   * @param {import('./world.js').World} world
   * @param {import('./mobEntityCollisions.js').LandMobColliderList | Array<{ min: number[], max: number[] }> | null | undefined} mobColliders Land mob AABBs (blocks movement like terrain)
   */
  update(world, dt, input, yaw, mobColliders = null) {
    ...
    if (inLava) {
      ...
      } else if (input.swimDown) {
        this.vy += GRAVITY_WATER * dt;
        this.vy -= SWIM_DOWN_ACCEL * dt;
      } else {
        /* Slow sink when not trying to rise or dive faster. */
        this.vy += WATER_IDLE_SINK_GRAVITY * dt;
        if (this.vy < WATER_IDLE_SINK_CAP_VY) this.vy = WATER_IDLE_SINK_CAP_VY;
      }
    ...
    this.tryMove(world, dt, this.vx * dt, 0, 0, mobColliders);
    if (!inWater && !inLava && this.vy < 0) {
      this._maxFallSpeed = Math.max(this._maxFallSpeed, -this.vy);
    }
    this.tryMove(world, dt, 0, this.vy * dt, 0, mobColliders);
    this.tryMove(world, dt, 0, 0, this.vz * dt, mobColliders);
    ...
    if (this.fallDamageThisFrame > 0 && aabbOverlapsFluid(world, m2, mx2) && !aabbOverlapsBlockId(world, m2, mx2, BlockId.LAVA)) {
      this.fallDamageThisFrame = 0;
    }
  ...
  /**
   * @param {import('./world.js').World} world
   * @param {import('./mobEntityCollisions.js').LandMobColliderList | Array<{ min: number[], max: number[] }> | null | undefined} mobColliders
   */
  tryMove(world, dt, dx, dy, dz, mobColliders = null) {
    const blocks = (min: number[], max: number[]) =>
      collidesWorldOrMobBoxes(world, min, max, mobColliders ?? undefined);
