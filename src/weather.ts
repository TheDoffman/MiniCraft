...
import { biomeParam, getBiome, BIOME, hashSeed } from './worldgen';
 ...
 * Mutates `cur` in place to avoid per-frame object allocation.
...
export function smoothWeatherSample(cur, target, dt, rate = 2.8) {
  ...
  cur.strength = cur.strength + (target.strength - cur.strength) * k;
  cur.fogAdd = cur.fogAdd + (target.fogAdd - cur.fogAdd) * k;
  cur.skyDim = cur.skyDim + (target.skyDim - cur.skyDim) * k;
  cur.cloudDarken = cur.cloudDarken + (target.cloudDarken - cur.cloudDarken) * k;
  cur.biome = target.biome;
  return cur;
}
