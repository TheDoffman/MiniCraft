...
import { BLOCKS, TILE_PX, ATLAS_TILES } from './blocktypes';
...
/** One compiled shader; each burst uses a clone with its own opacity uniform. */
let burstMaterialTemplate = /** @type {THREE.ShaderMaterial | null} */ (null);

function getBurstMaterialClone() {
  if (!burstMaterialTemplate) {
    burstMaterialTemplate = makeBurstMaterial();
  }
  return burstMaterialTemplate.clone();
}
...
export function addBlockBreakBurst(scene, bx, by, bz, blockId) {
  ...
  const mat = getBurstMaterialClone();
  if (rendererRef) {
