...
/** Solid blocks the ray stops on (includes bedrock; excludes fluids). Open doors pass through like {@link isParticleOccluder}. */
export function isRayStop(world, x, y, z) {
  const id = world.get(x, y, z);
  if (id === 0) return false;
  const def = blockDef(id);
  if (def.torch) return true;
  if (def.tallGrassBottom || def.tallGrassTop || def.shortGrass || def.farmCrop) return true;
  if (def.fluid || !def.solid) return false;
  if (def.collision === false) return false;
  return true;
}
