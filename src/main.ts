...
import { buildRegionMesh } from './mesher';
import {
  buildAtlasCanvas,
...
import { buildSelectionOutlineGeometry } from './selectionOutline';
import { aabbFullySubmergedInFluid } from './physics';
import {
  buildLandMobColliderBoxes,
  resolveLandMobEntityCollisions,
} from './mobEntityCollisions';
import { CHUNK_XZ, regionForChunk, chunkKeysForBlock } from './chunks';
...
import { createPlayerModel, disposePlayerModel, updatePlayerModelAnimation, setPlayerNameTag } from './playerModel';
import { createInventoryPlayerPreview } from './inventoryPlayerPreview';
import { initMainMenuTerrain } from './mainMenuTerrain';
import { initMainMenuClouds } from './mainMenuClouds';
import {
...
import { sampleWeather, smoothWeatherSample } from './weather';
import {
...
import {
  computeRainExposureAndShelterCached,
  rainComfortTtlFrames,
} from './weatherSoundExposure';
import {
  waterShimmerUniforms,
  attachWaterDepthMurkShader,
  attachLavaDepthMurkShader,
} from './fluidMaterials';
import { computeCaveLightFactors } from './caveLighting';
import {
...
import {
  ...
  raycastNearestSheepWithT,
  raycastNearestChickenWithT,
  ...
  spawnSheepAroundWorld,
  spawnChickensAroundWorld,
  spawnSquidsInWater,
  spawnZombiesAroundPlayer,
  restoreMobsFromSave,
  serializeMobsState,
  disposeMobsSharedResources,
  disposeZombieMobGroup,
} from './mobs';
import { createGroundDrop, tickGroundDrops } from './mobDrops';
import { disposePigMaterialBundle } from './pigTextures';
import { disposeCowMaterialBundle } from './cowTextures';
import { disposeSheepMaterialBundle } from './sheepTextures';
import { disposeChickenMaterialBundle } from './chickenTextures';
import { disposeSquidMaterialBundle } from './squidTextures';
import { disposeZombieMaterialBundle } from './zombieTextures';
import { playSound, startAmbient, updateAmbient, setMasterVolume } from './sound';
...
import {
  initCaveDustParticles,
  updateCaveDustParticles,
  disposeCaveDustParticles,
} from './caveDustParticles';
import { torchSupportDeltaToAttach, TorchAttach } from './torchAttach';
import { recomputeWaterAround, needsWaterRecomputeAfterBreak } from './waterFlow';
import {
  WORLD_H,
  CHUNK_MESH_MARGIN,
  MAX_HEALTH,
  ...
  BEEF_HUNGER_RESTORE,
  MUTTON_HUNGER_RESTORE,
  RAW_CHICKEN_HUNGER_RESTORE,
  STEAK_HUNGER_RESTORE,
  COOKED_PORKCHOP_HUNGER_RESTORE,
  COOKED_MUTTON_HUNGER_RESTORE,
  COOKED_CHICKEN_HUNGER_RESTORE,
  ROTTEN_FLESH_HUNGER_RESTORE,
  ...
  TARGET_COW_COUNT,
  TARGET_SHEEP_COUNT,
  TARGET_CHICKEN_COUNT,
  TARGET_SQUID_COUNT,
  TARGET_ZOMBIE_COUNT_NIGHT,
  BASE_MINE_TIME,
  UNDERWATER_MINE_TIME_MULTIPLIER,
  REACH,
  FALL_SHAKE_MAX,
  FALL_SHAKE_DECAY,
...
/** Increments each frame while the sim loop runs — weather exposure cache, etc. */
let simFrameSeq = 0;
let cropGrowthAccum = 0;
const playerPosScratch = { x: 0, y: 0, z: 0 };
const _rainShelterScratch = { gainMul: 1, muffle: 0 };
const _foodTable: Record<number, [number, string]> = {
  [BlockId.ROTTEN_FLESH]: [ROTTEN_FLESH_HUNGER_RESTORE, 'Rotten Flesh'],
  [BlockId.BEEF]: [BEEF_HUNGER_RESTORE, 'Beef'],
  [BlockId.PORKCHOP]: [PORKCHOP_HUNGER_RESTORE, 'Porkchop'],
  [BlockId.MUTTON]: [MUTTON_HUNGER_RESTORE, 'Mutton'],
  [BlockId.RAW_CHICKEN]: [RAW_CHICKEN_HUNGER_RESTORE, 'Raw Chicken'],
  [BlockId.STEAK]: [STEAK_HUNGER_RESTORE, 'Steak'],
  [BlockId.COOKED_PORKCHOP]: [COOKED_PORKCHOP_HUNGER_RESTORE, 'Cooked Porkchop'],
  [BlockId.COOKED_MUTTON]: [COOKED_MUTTON_HUNGER_RESTORE, 'Cooked Mutton'],
  [BlockId.COOKED_CHICKEN]: [COOKED_CHICKEN_HUNGER_RESTORE, 'Cooked Chicken'],
  [BlockId.BREAD]: [BREAD_HUNGER_RESTORE, 'Bread'],
};

/** @type {Worker | null} */
let chunkMeshWorker = null;
try {
  chunkMeshWorker = new Worker(new URL('./chunkMeshWorker.ts', import.meta.url), { type: 'module' });
} catch {
...
/** Title-screen sky clouds (2D); no-op dispose when unused. */
let disposeMainMenuClouds = () => {};
const worldGenScreen = document.getElementById('worldGenScreen');
const worldGenFill = document.getElementById('worldGenFill');
const worldGenLabel = document.getElementById('worldGenLabel');
const worldGenEnterBtn = document.getElementById('worldGenEnterBtn');
let worldGenActive = false;
let worldGenInitialBacklog = 0;
/** True when chunks are done but the user hasn't clicked into the world yet. */
let worldGenWaitingForClick = false;
...
let atlasDrawSource: CanvasImageSource = _atlasCanvas;
...
function swapBlockAtlasFromFile(/** @type {THREE.Texture} */ loadedTex) {
  ...
  if (held instanceof THREE.Mesh && held.material && !Array.isArray(held.material)) {
    held.material.map = loadedTex;
    held.material.needsUpdate = true;
  }
  const tpm = playerModel.getObjectByName('tpHeldBlock');
  if (tpm instanceof THREE.Mesh && tpm.material && !Array.isArray(tpm.material)) {
...
const mainMenuSky = document.getElementById('mainMenuSky');
const mainMenuCloudCanvas = document.getElementById('mainMenuCloudCanvas');
if (
  mainMenuSky instanceof HTMLElement &&
  mainMenuCloudCanvas instanceof HTMLCanvasElement &&
  startEl &&
  !window.matchMedia('(prefers-reduced-motion: reduce)').matches
) {
  disposeMainMenuClouds = initMainMenuClouds(mainMenuCloudCanvas, mainMenuSky, startEl);
}
...
/** Matches `player.inWater` for the current mining session (reset when it changes). */
let miningSessionUnderwater = false;
let mouseLeftDown = false;

/** Calculate how long to mine a block given held item. */
function getMineTime(blockId, heldBlockId, underwater = false) {
  ...
  let t = Math.max(0.05, (hardness * BASE_MINE_TIME) / speed);
  if (underwater) t *= UNDERWATER_MINE_TIME_MULTIPLIER;
  return t;
}
...
let drops = [];
spawnPigsAroundWorld(world, scene, pigs, 14, undefined, undefined);
spawnCowsAroundWorld(world, scene, cows, 10, undefined, undefined);
spawnSheepAroundWorld(world, scene, sheep, TARGET_SHEEP_COUNT, undefined, undefined);
spawnChickensAroundWorld(world, scene, chickens, TARGET_CHICKEN_COUNT, undefined, undefined);
spawnSquidsInWater(world, scene, squids, 10, undefined, undefined);
...
const CHUNK_BUILD_MAX_PER_FRAME_PLAY = 1;
...
const CHUNK_BUILD_TIME_BUDGET_MS_PLAY = 1.8;
/** Slightly more time when far behind so the world catches up without 8-chunk spikes. */
const CHUNK_BUILD_TIME_BUDGET_MS_PLAY_CATCHUP = 2.4;
...
/**
 * Reusable Set for the wanted-chunk keys; avoids allocating a new Set each call.
 * @type {Set<string>}
 */
const _wantedChunkKeys = new Set();
...
function updateChunkDrawVisibility() {
  ...
  if (
    pcx === _lastVisPcx &&
    pcz === _lastVisPcz &&
    chunkMeshes.size === _lastVisSize &&
    chunkVisibilityEpoch === _lastVisEpoch
  ) return;
  _lastVisPcx = pcx;
  _lastVisPcz = pcz;
  _lastVisSize = chunkMeshes.size;
  _lastVisEpoch = chunkVisibilityEpoch;
...
/**
 * Reusable array for sorting the build queue; avoids spreading the Set every frame.
 * @type {string[]}
 */
let _pendingSortBuf = [];
let _pendingSortedPcx = NaN;
let _pendingSortedPcz = NaN;
let _pendingSortedSize = -1;
...
function processChunkBuildQueue(maxChunks, budgetMs) {
  ...
  const needResort =
    pcx !== _pendingSortedPcx ||
    pcz !== _pendingSortedPcz ||
    chunkBuildPending.size !== _pendingSortedSize;

  if (needResort) {
    _pendingSortBuf.length = 0;
    for (const k of chunkBuildPending) _pendingSortBuf.push(k);
    _pendingSortBuf.sort((ka, kb) => {
      const ca = ka.indexOf(',');
      const cb = kb.indexOf(',');
    ...
    _pendingSortedPcx = pcx;
    _pendingSortedPcz = pcz;
    _pendingSortedSize = chunkBuildPending.size;
  }

  const t0 = performance.now();
  const deadline = t0 + budgetMs;
  let built = 0;
  for (let i = 0; i < _pendingSortBuf.length; i++) {
    if (built >= maxChunks || performance.now() >= deadline) break;
    const bestKey = _pendingSortBuf[i];
...
function applyWorld(newWorld: World, mobData: 'spawn' | SavedMobBlob = 'spawn') {
  clearMobsFromScene(scene, pigs, cows, squids, drops, zombies, sheep, chickens);
  disposeAllChunks();
  furnaceStates.clear();
  furnaceOpen = false;
  openFurnaceKey = '';
  ...
  if (mobData && typeof mobData === 'object') {
    restoreMobsFromSave(scene, atlasTex, pigs, cows, squids, drops, mobData, zombies, world, sheep, chickens);
  } else {
    spawnPigsAroundWorld(world, scene, pigs, 14, player.x, player.z);
    spawnCowsAroundWorld(world, scene, cows, 10, player.x, player.z);
    spawnSheepAroundWorld(world, scene, sheep, TARGET_SHEEP_COUNT, player.x, player.z);
    spawnChickensAroundWorld(world, scene, chickens, TARGET_CHICKEN_COUNT, player.x, player.z);
...
let invSlots: InvSlot[] = createInventory();
/** @type {import('./inventory.js').InvSlot[]} */
...
const furnaceStates = new Map<
  string,
  { input: InvSlot; fuel: InvSlot; output: InvSlot; burnLeft: number; smeltProgress: number }
>();

/* ── Cursor item (Minecraft-style click-to-move inventory) ── */
let cursorItem: InvSlot = { blockId: 0, count: 0 };
...
function invSlotShiftClick(slotIdx) {
  ...
  if (!isTool) {
    for (let i = lo; i <= hi && slot.count > 0; i++) {
      const t = invSlots[i];
      if (t.blockId === bid && t.count < MAX_STACK) {
        const move = Math.min(slot.count, MAX_STACK - t.count);
        t.count += move;
        slot.count -= move;
      }
    }
  ...
  for (let i = lo; i <= hi && slot.count > 0; i++) {
    if (isEmptySlot(invSlots[i])) {
      const move = isTool ? 1 : slot.count;
      invSlots[i].blockId = bid;
      invSlots[i].count = move;
...
function updateStatBarsUI() {
  ...
  if (h === statBarsLastH && s === statBarsLastS && f === statBarsLastF) return;
  statBarsLastH = h;
  statBarsLastS = s;
  statBarsLastF = f;
...
const _eyePosScratch = new THREE.Vector3();
function eyePos() {
  return _eyePosScratch.set(
    player.x,
...
function syncPlayerModel(dt) {
  ...
  const nameTag = playerModel.userData.nameTag;
  if (nameTag) nameTag.visible = cameraViewMode !== 0;
...
function updateDayNight(dt) {
  ...
  smoothWeatherSample(weatherDisplay, targetWx, dt);
  ...
  (scene.background as THREE.Color).copy(_skyScratch);
  ...
  _moonPosScratch.copy(sun.target.position).multiplyScalar(2).sub(sun.position);
...
function updateSelection(blockHit) {
  lastRay = blockHit;
  if (lastRay) {
    ...
    if (key !== selectionOutlineKey) {
      ...
      // buildSelectionOutlineGeometry returns EdgesGeometry from arbitrary block shape
      selection.geometry = nextGeom as unknown as typeof selection.geometry;
    }
...
function tryHitMob() {
  ...
  const sheepHit = raycastNearestSheepWithT(ox, oy, oz, dx, dy, dz, REACH, sheep);
  const chickenHit = raycastNearestChickenWithT(ox, oy, oz, dx, dy, dz, REACH, chickens);
  ...
  const sheepT = sheepHit ? sheepHit.t : Infinity;
  const chickenT = chickenHit ? chickenHit.t : Infinity;
  ...
  const mobT = Math.min(pigT, cowT, sheepT, chickenT, squidT, zombieT);
  if (mobT <= blockT) {
    const dmg = getHeldAttackDamage();
    /** @type {{ t: number, hit: () => void }[]} */
    const candidates = [];
    if (zombieHit) candidates.push({ t: zombieT, hit: () => damageZombie(scene, zombies, dmg, zombieHit.zombie, dx, dz) });
    if (pigHit) candidates.push({ t: pigT, hit: () => damageNearestPig(scene, pigs, dmg, atlasTex, drops, pigHit.pig, dx, dz) });
    if (cowHit) candidates.push({ t: cowT, hit: () => damageNearestCow(scene, cows, dmg, atlasTex, drops, cowHit.cow, dx, dz) });
    if (sheepHit) candidates.push({ t: sheepT, hit: () => damageNearestSheep(scene, sheep, dmg, atlasTex, drops, sheepHit.sheep, dx, dz) });
    if (chickenHit)
      candidates.push({
        t: chickenT,
        hit: () => damageNearestChicken(scene, chickens, dmg, atlasTex, drops, chickenHit.chicken, dx, dz),
      });
    if (squidHit) candidates.push({ t: squidT, hit: () => damageSquid(scene, squids, dmg, atlasTex, drops, squidHit.squid, dx, dz) });
    let best = candidates[0];
    for (let i = 1; i < candidates.length; i++) {
      if (candidates[i].t < best.t) best = candidates[i];
    }
    if (best && Math.abs(best.t - mobT) < 1e-5) {
      best.hit();
      damageHeldToolMelee();
...
function completeBlockBreak(x, y, z) {
  ...
  if (blockId === 0 || blockDef(blockId).indestructible) return;
  if (blockId === BlockId.FURNACE) {
    ...
    if (fs) {
      for (const part of ['input', 'fuel', 'output']) {
        const s = fs[part];
        if (!isEmptySlot(s)) {
          const leftover = addItemToInventory(invSlots, s.blockId, s.count, {
            hotbarOnly: gameMode === 'creative',
          });
          if (leftover > 0) {
            const gd = createGroundDrop(x + 0.5, y + 0.5, z + 0.5, s.blockId, leftover, atlasTex);
            gd.vy = 2.0;
            scene.add(gd.mesh);
            drops.push(gd);
          }
  ...
  if (gameMode === 'survival') {
    ...
    if (
      ...
      addItemToInventory(invSlots, BlockId.WHEAT_SEEDS, 1, undefined);
    }
    if (blockId === BlockId.WHEAT_3) {
      addItemToInventory(invSlots, BlockId.WHEAT_SEEDS, 1, undefined);
      if (Math.random() < 0.45) {
        addItemToInventory(invSlots, BlockId.WHEAT_SEEDS, 1, undefined);
      }
...
function updateMining(dt, blockHit) {
  ...
  const blockId = world.get(x, y, z);
  if (blockId === 0 || blockDef(blockId).indestructible) {
    miningBlock = null;
    miningProgress = 0;
    crackMesh.visible = false;
    return;
  ...
  const key = `${x},${y},${z}`;
  const underwater = player.inWater;
  if (miningBlock !== key) {
    miningBlock = key;
    miningProgress = 0;
    miningSessionUnderwater = underwater;
    const heldId = getHotbarHeldToolId();
    miningTimeForBlock = getMineTime(blockId, heldId, miningSessionUnderwater);
  } else if (underwater !== miningSessionUnderwater) {
    miningProgress = 0;
    miningSessionUnderwater = underwater;
    const heldId = getHotbarHeldToolId();
    miningTimeForBlock = getMineTime(blockId, heldId, miningSessionUnderwater);
  }
  if (miningTimeForBlock <= 0) {
    crackMesh.visible = false;
...
function cropHasSkyLight(world, x, y, z) {
  for (let yy = y + 1; yy < world.height; yy++) {
    const tid = world.get(x, yy, z);
    if (tid === 0) continue;
    const d = blockDef(tid);
    if (d.fluid) continue;
    if (d.solid && d.collision !== false) return false;
  }
  return true;
}

function tickCropGrowthNearPlayer(dt) {
  cropGrowthAccum += dt;
  if (cropGrowthAccum < 2.2) return;
  cropGrowthAccum = 0;
  const pauseOpen = pauseMenuEl && !pauseMenuEl.classList.contains('hidden');
  if (!world || pauseOpen || playerDead) return;
  const px = Math.floor(player.x);
  const pz = Math.floor(player.z);
  const r = 22;
  for (let i = 0; i < 28; i++) {
    const x = px + (((Math.random() * 2 - 1) * r) | 0);
    const z = pz + (((Math.random() * 2 - 1) * r) | 0);
    const sy = world.topSolidY(x, z);
    if (sy < 1) continue;
    const y = sy + 1;
    if (y >= world.height) continue;
    const id = world.get(x, y, z);
    if (!isWheatCropId(id) || id === BlockId.WHEAT_3) continue;
    if (!cropHasSkyLight(world, x, y, z)) continue;
    if (Math.random() < 0.38) {
      world.set(x, y, z, id + 1);
      rebuildChunksForBlock(x, z);
    }
  }
}

const _minimapRgbLut = new Uint8Array(256 * 3);
{
  const DEFAULT_R = 88, DEFAULT_G = 88, DEFAULT_B = 88;
  for (let i = 0; i < 256; i++) {
    _minimapRgbLut[i * 3] = DEFAULT_R;
    _minimapRgbLut[i * 3 + 1] = DEFAULT_G;
    _minimapRgbLut[i * 3 + 2] = DEFAULT_B;
  }
  const set = (id: number, r: number, g: number, b: number) => {
    _minimapRgbLut[id * 3] = r;
    _minimapRgbLut[id * 3 + 1] = g;
    _minimapRgbLut[id * 3 + 2] = b;
  };
  set(BlockId.GRASS, 72, 140, 58);
  set(BlockId.DIRT, 120, 85, 52);
  set(BlockId.FARMLAND, 120, 85, 52);
  set(BlockId.STONE, 110, 110, 110);
  set(BlockId.COBBLE, 110, 110, 110);
  set(BlockId.SAND, 210, 200, 130);
  set(BlockId.SANDSTONE, 210, 200, 130);
  set(BlockId.WATER, 48, 72, 200);
  set(BlockId.LOG, 90, 70, 45);
  set(BlockId.LEAVES, 50, 110, 48);
  set(BlockId.SNOW, 220, 230, 245);
  set(BlockId.ICE, 220, 230, 245);
  set(BlockId.LAVA, 255, 90, 20);
}

let _minimapLastX = NaN;
let _minimapLastZ = NaN;
let _minimapCtx = /** @type {CanvasRenderingContext2D | null} */ (null);
let _minimapImageData: ImageData | null = null;
const MINIMAP_RES = 96;

/** Highest meaningful block in the column (fluids and solid colliders from the sky down — shows surface water reliably). */
function minimapColumnSurfaceBlockId(wx: number, wz: number) {
  for (let y = world.height - 1; y >= 0; y--) {
    const id = world.get(wx, y, wz);
    if (id === 0) continue;
    const def = blockDef(id);
    if (def.fluid) return id;
    if (def.solid && def.collision !== false) return id;
    return id;
  }
  return 0;
}
...
function updateMinimap() {
  ...
  const px = Math.floor(player.x);
  const pz = Math.floor(player.z);
  if (px === _minimapLastX && pz === _minimapLastZ) return;
  _minimapLastX = px;
  _minimapLastZ = pz;

  if (!_minimapCtx) {
    el.width = MINIMAP_RES;
    el.height = MINIMAP_RES;
    _minimapCtx = el.getContext('2d');
  }
  const ctx = _minimapCtx;
  if (!ctx) return;
  const res = MINIMAP_RES;
  if (!_minimapImageData) _minimapImageData = ctx.createImageData(res, res);
  const d = _minimapImageData.data;
  const lut = _minimapRgbLut;
  let p = 0;
  for (let yi = 0; yi < res; yi++) {
    for (let xi = 0; xi < res; xi++) {
      const wx = px + xi - (res >> 1);
      const wz = pz + yi - (res >> 1);
      const bid = minimapColumnSurfaceBlockId(wx, wz);
      const off = bid * 3;
      d[p++] = lut[off];
      d[p++] = lut[off + 1];
      d[p++] = lut[off + 2];
  ...
  ctx.lineWidth = 1.5;
  ctx.beginPath();
  ctx.arc(res / 2, res / 2, 2.2, 0, Math.PI * 2);
  ctx.stroke();
}
...
function tryTillSoil() {
  ...
  const held = slot.blockId;
  if (TOOL_INFO[held]?.type !== 'shovel') return false;
  const { x, y, z } = lastRay.hit;
  const tid = world.get(x, y, z);
  if (tid !== BlockId.GRASS && tid !== BlockId.DIRT) return false;
  world.set(x, y, z, BlockId.FARMLAND);
  rebuildChunksForBlock(x, z);
  if (gameMode === 'survival') {
    const broke = damageToolInSlot(slot, 1);
    refreshInventoryUI();
...
function tryPlaceTorch() {
  ...
  const supportDef = blockDef(supportId);
  if (supportDef.fluid) return false;
...
function tryPlace() {
  ...
  if (id === BlockId.WHEAT_SEEDS) {
    const { x, y, z } = lastRay.prev;
    if (!world.inBounds(x, y, z)) return;
    if (world.get(x, y, z) !== 0) return;
    if (world.get(x, y - 1, z) !== BlockId.FARMLAND) return;
    world.set(x, y, z, BlockId.WHEAT_0);
    rebuildChunksForBlock(x, z);
    if (gameMode === 'survival') {
      consumeFromSlot(invSlots, slotIdx, 1);
    }
...
function dropHeldItem() {
  ...
  const fz = -Math.cos(yaw) * Math.cos(pitch);
  const spawnDist = 0.8;
  const sx = player.x + fx * spawnDist;
  const sy = player.y + player.eyeHeight - 0.2 + fy * spawnDist;
  const sz = player.z + fz * spawnDist;

  const gd = createGroundDrop(sx, sy, sz, blockId, 1, atlasTex);
  gd.vy = 2.0;
  gd.vx = fx * 4.0;
  gd.vz = fz * 4.0;
  gd.pickupDelay = 3.0;
...
function tryEat() {
  ...
  const slot = invSlots[slotIdx];
  if (isEmptySlot(slot)) return false;
  const entry = _foodTable[slot.blockId];
  ...
  const [hungerAmt, label] = entry;
  consumeFromSlot(invSlots, slotIdx, 1);
  playerHunger = Math.min(MAX_HUNGER, playerHunger + hungerAmt);
  updateStatBarsUI();
  refreshInventoryUI();
  showToast(`Ate ${label} (+${hungerAmt} hunger)`);
  playSound('eat');
  return true;
}
...
let lastFurnaceSlotSig = '';

function furnaceSlotsSignature(fs) {
  return `${fs.input.blockId},${fs.input.count}:${fs.fuel.blockId},${fs.fuel.count}:${fs.output.blockId},${fs.output.count}`;
}

function refreshFurnacePanel() {
  if (!furnaceOpen || !openFurnaceKey || !furnaceStates.has(openFurnaceKey)) return;
  const fs = furnaceStates.get(openFurnaceKey)!;
  const sig = furnaceSlotsSignature(fs);
  const rebuild = sig !== lastFurnaceSlotSig;
  lastFurnaceSlotSig = sig;
  if (!rebuild) {
  ...
  const mounts: [HTMLElement | null, InvSlot, 'input' | 'fuel' | 'output'][] = [
...
function flushCraftTableGridToInventory() {
  ...
  for (let i = 0; i < 9; i++) {
    if (craftTableGrid[i] > 0) {
      const leftover = addItemToInventory(invSlots, craftTableGrid[i], 1, { hotbarOnly });
      if (leftover > 0) {
        const gd = createGroundDrop(player.x, player.y + 0.5, player.z, craftTableGrid[i], leftover, atlasTex);
        gd.vy = 1.5;
        scene.add(gd.mesh);
        drops.push(gd);
      }
    ...
    wrap.addEventListener('click', (e) => {
      ...
      if (isEmptySlot(cursorItem)) {
        ...
        if (cursorItem.count <= 0) {
          ...
          addItemToInventory(invSlots, oldId, 1, undefined);
        }
        updateCursorItemDisplay();
        snapCursorItemElToEvent(e);
        refreshCraftingTablePanel();
...
function tryInteractBlock() {
  ...
  if (id === BlockId.DOOR || id === BlockId.DOOR_OPEN) {
    ...
    if (topId === BlockId.DOOR_TOP || topId === BlockId.DOOR_OPEN_TOP) {
      ...
      world.set(x, y + 1, z, open ? BlockId.DOOR_OPEN_TOP : BlockId.DOOR_TOP);
      world.set(x, y, z, open ? BlockId.DOOR_OPEN : BlockId.DOOR);
    }
    rebuildChunksForBlock(x, z);
    playSound('place');
    return true;
...
function flushCraftGridToInventory() {
  ...
  for (let i = 0; i < 4; i++) {
    if (craftGrid[i] > 0) {
      const leftover = addItemToInventory(invSlots, craftGrid[i], 1, { hotbarOnly });
      if (leftover > 0) {
        const gd = createGroundDrop(player.x, player.y + 0.5, player.z, craftGrid[i], leftover, atlasTex);
        gd.vy = 1.5;
        scene.add(gd.mesh);
        drops.push(gd);
      }
...
function doSave() {
  try {
    saveToLocalStorage(
      world,
      invSlots,
      serializeMobsState(pigs, cows, squids, drops, zombies, sheep, chickens),
      armorSlots,
...
function doLoad() {
  ...
  if (state.player) {
    ...
    if (typeof state.player.yaw === 'number') yaw = state.player.yaw;
    if (typeof state.player.pitch === 'number') pitch = state.player.pitch;
  }
  ...
  setPlayerNameTag(playerModel, readPlayerName());
  syncModeButtons();
  showWorldGenScreen();
...
window.addEventListener('keydown', (e) => {
  ...
  if (e.code >= 'Digit1' && e.code <= 'Digit9') {
    ...
    const isInputFocused = ae instanceof HTMLInputElement || ae instanceof HTMLTextAreaElement;
    if (!isInputFocused) {
  ...
  if (e.code === 'KeyQ' && pointerLocked && !invOpen && !worldGuiOpen() && !playerDead) {
    dropHeldItem();
    e.preventDefault();
  }
...
const SETTINGS_UI_CLUSTERS = [
  {
    ...
    weatherEffects: 'settingsWeatherEffects',
    ...
    showMinimap: 'settingsShowMinimap',
    ambientMusic: 'settingsAmbientMusic',
  },
  {
    ...
    weatherEffects: 'pauseSettingsWeatherEffects',
    ...
    showMinimap: 'pauseSettingsShowMinimap',
    ambientMusic: 'pauseSettingsAmbientMusic',
  },
];
...
function syncSettingsUiFromState() {
  for (const ids of SETTINGS_UI_CLUSTERS) {
    ...
    const wqEl = document.getElementById(ids.weatherEffects);
    if (wqEl instanceof HTMLSelectElement) wqEl.value = gameSettings.weatherEffectsQuality;
    ...
    const mm = document.getElementById(ids.showMinimap);
    if (mm instanceof HTMLInputElement) mm.checked = gameSettings.showMinimap;
    const amb = document.getElementById(ids.ambientMusic);
    if (amb instanceof HTMLInputElement) amb.checked = gameSettings.ambientMusicBed;
  }
}
...
function bindSettingsPanel() {
  for (const ids of SETTINGS_UI_CLUSTERS) {
    ...
    const wqEl = document.getElementById(ids.weatherEffects);
    if (wqEl instanceof HTMLSelectElement) {
      wqEl.addEventListener('change', () => {
        const v = wqEl.value;
        if (v !== 'full' && v !== 'balanced' && v !== 'minimal') return;
        gameSettings = saveSettings({ weatherEffectsQuality: v });
        syncSettingsUiFromState();
      });
    }
    const tlEl = document.getElementById(ids.maxTorchLights);
    ...
    bindCheck(ids.showMinimap, 'showMinimap');
    bindCheck(ids.ambientMusic, 'ambientMusicBed');
  }
  syncSettingsUiFromState();
  applyGraphicsSettingsFromState();
}
...
canvas.addEventListener(
  ...
  (e) => {
    ...
    if (e.button === 2) {
      e.preventDefault();
      if (!tryEat() && !tryInteractBlock() && !tryTillSoil()) tryPlace();
    }
  },
  { passive: false },
);
...
function showWorldGenScreen() {
  worldGenActive = true;
  worldGenWaitingForClick = false;
  worldGenInitialBacklog = Math.max(1, chunkBuildPending.size);
  if (worldGenScreen) {
    worldGenScreen.classList.remove('hidden');
    worldGenScreen.setAttribute('aria-hidden', 'false');
  }
  worldGenEnterBtn?.classList.add('hidden');
  if (worldGenFill) worldGenFill.style.width = '0%';
  if (worldGenLabel) worldGenLabel.textContent = 'Building terrain...';
...
function hideWorldGenScreen() {
  worldGenActive = false;
  worldGenWaitingForClick = false;
  if (worldGenScreen) {
    worldGenScreen.classList.add('hidden');
    worldGenScreen.setAttribute('aria-hidden', 'true');
  }
  worldGenEnterBtn?.classList.add('hidden');
}

function worldGenChunksReady() {
  worldGenWaitingForClick = true;
  if (worldGenFill) worldGenFill.style.width = '100%';
  if (worldGenLabel) worldGenLabel.textContent = 'World ready!';
  worldGenEnterBtn?.classList.remove('hidden');
  worldGenEnterBtn?.focus();
}

function finalizeEnterWorld() {
  hideWorldGenScreen();
  syncStatBarsVisibility();
  refreshInventoryUI();
...
worldGenEnterBtn?.addEventListener('click', () => {
  finalizeEnterWorld();
});
...
startEl?.addEventListener('keydown', (e) => {
  if (e.code !== 'Enter' || !startEl || startEl.classList.contains('hidden')) return;
  const t = e.target;
  if (!(t instanceof HTMLInputElement)) return;
  if (t.type === 'range' || t.type === 'checkbox' || t.type === 'radio' || t.type === 'button') return;
  if (t.id !== 'seedInput' && t.id !== 'playerNameInput') return;
  e.preventDefault();
  startPlay();
});
...
document.addEventListener('pointerlockchange', () => {
  ...
  if (pointerLocked) {
    ...
    if (!invOpen && !worldGuiOpen() && !pendingPointerLock && !playerDead && !worldGenActive) {
      if (inGame) {
        showPauseMenu();
      } else {
        hidePauseMenu();
...
function disposeEngine() {
  chunkMeshWorker?.terminate();
  chunkMeshWorker = null;
  disposeMainMenuClouds();
  disposeMainMenuClouds = () => {};
  ...
  disposeCaveDustParticles(scene);
...
function loop() {
  ...
  const pauseOpen = !!(pauseMenuEl && !pauseMenuEl.classList.contains('hidden'));
  const simPaused = !startHidden && pauseOpen;
  ...
  let rainAtmosphere = null;
  if (startHidden && world) {
    const ttl = rainComfortTtlFrames(gameSettings.weatherEffectsQuality);
    rainAtmosphere = computeRainExposureAndShelterCached(
      world,
      player.y + player.eyeHeight,
      Math.floor(player.x),
      Math.floor(player.z),
  ...
  if ((pointerLocked || invOpen) && !worldGuiOpen()) {
    ...
    if (player.flying && gameMode === 'creative') {
      ...
      const landMobColliders = buildLandMobColliderBoxes(pigs, cows, sheep, chickens, zombies, squids);
      player.update(world, dt, keys, yaw, landMobColliders);
    }
    ...
    if (isSurvival) {
      ...
      if (playerHunger < HUNGER_STARVE_THRESHOLD) {
        playerHealth = Math.max(0, playerHealth - STARVE_DAMAGE_PER_SEC * dt);
      }
    }
          ...
          const reason =
            player.inLava && !player.flying
              ...
              : player.fallDamageThisFrame > 0
                ...
                : playerHunger < HUNGER_STARVE_THRESHOLD
                  ? 'Starved to death'
                  : 'Died';
  ...
  if (startEl?.classList.contains('hidden')) {
    const rainParticleExposure = rainAtmosphere?.exposure ?? 1;
    updateWeatherParticles(
      weatherParticles,
      dt,
      camera,
      ...
      {
        simFrame: simFrameSeq,
        quality: gameSettings.weatherEffectsQuality,
      },
    );
  } else {
  ...
  /** Title, pause, or world-gen loading: still stream chunk meshes aggressively. */
  ...
  if (worldGenActive && !worldGenWaitingForClick) {
    const built = worldGenInitialBacklog - backlog;
    const pct = Math.min(100, Math.round((built / worldGenInitialBacklog) * 100));
    if (worldGenFill) worldGenFill.style.width = `${pct}%`;
    if (worldGenLabel) {
      if (pct < 40) worldGenLabel.textContent = 'Building terrain...';
      else if (pct < 75) worldGenLabel.textContent = 'Planting trees...';
      else worldGenLabel.textContent = 'Almost ready...';
    }
    if (backlog === 0 || pct >= 100) {
      worldGenChunksReady();
    }
  }
  ...
  if (worldSimActive) {
    ...
    for (let i = sheep.length - 1; i >= 0; i--) {
      updateSheep(sheep[i], world, dt);
      if (sheep[i].pendingDeathCleanup) {
        finalizeDeadSheep(scene, sheep, i, atlasTex, drops, world);
      }
    }
    for (let i = chickens.length - 1; i >= 0; i--) {
      updateChicken(chickens[i], world, dt);
      if (chickens[i].pendingEgg) {
        chickens[i].pendingEgg = false;
        const c = chickens[i];
        const gd = createGroundDrop(c.x, c.y + 0.2, c.z, BlockId.EGG, 1, atlasTex);
        scene.add(gd.mesh);
        drops.push(gd);
        playSound('pop');
      }
      if (chickens[i].pendingDeathCleanup) {
        finalizeDeadChicken(scene, chickens, i, atlasTex, drops, world);
      }
    }
    for (let i = squids.length - 1; i >= 0; i--) {
      updateSquid(squids[i], world, dt);
      if (squids[i].pendingDeathCleanup) {
        finalizeDeadSquid(scene, squids, i, atlasTex, drops);
      }
    }
    playerPosScratch.x = player.x;
    playerPosScratch.y = player.y;
    playerPosScratch.z = player.z;
    for (let i = zombies.length - 1; i >= 0; i--) {
      updateZombie(zombies[i], world, dt, playerPosScratch);
      ...
      if (!playerDead && (pointerLocked || invOpen) && gameMode === 'survival') {
        const dmg = zombieContactDamage(zombies[i], player, world);
        if (dmg > 0) {
    ...
    resolveLandMobEntityCollisions(world, player, {
      playerDead,
      playerFlying: player.flying && gameMode === 'creative',
      pigs,
      cows,
      sheep,
      chickens,
    ...
    if (mobRespawnTimer >= MOB_RESPAWN_INTERVAL) {
      ...
      if (sheep.length < TARGET_SHEEP_COUNT) {
        spawnSheepAroundWorld(world, scene, sheep, TARGET_SHEEP_COUNT - sheep.length, player.x, player.z);
      }
      if (chickens.length < TARGET_CHICKEN_COUNT) {
        spawnChickensAroundWorld(
          world,
          scene,
          chickens,
          TARGET_CHICKEN_COUNT - chickens.length,
          player.x,
          player.z,
        );
      }
    ...
    syncFirstPersonHandView();
    let sharedBlockHit = null;
    if (pointerLocked && !invOpen && !worldGuiOpen() && !playerDead && world) {
      const r = getInteractionRayOriginAndDir();
      sharedBlockHit = raycastBlocks(world, r.ox, r.oy, r.oz, r.dx, r.dy, r.dz, REACH);
    }
    updateMining(dt, sharedBlockHit);
    if (pointerLocked && !invOpen && !worldGuiOpen()) {
      updateSelection(sharedBlockHit);
    }
  }
  ...
  if (!menuOrPause && rainAtmosphere) {
    _rainShelterScratch.gainMul = rainAtmosphere.gainMul;
    _rainShelterScratch.muffle = rainAtmosphere.muffle;
  }
  const rainShelter = (!menuOrPause && rainAtmosphere) ? _rainShelterScratch : null;
  updateAmbient({
    ...
    rainShelter,
    ambientMusicBed: gameSettings.ambientMusicBed,
  });
  if (!simPaused) {
  ...
  }
  {
    const caveDustHud =
      ...
      !pauseOpen &&
      !worldGuiOpen();
    const dustInt = THREE.MathUtils.smoothstep(0.05, 0.78, 1 - caveSunMoonMulSm);
    updateCaveDustParticles(player, dt, player.inWater ? dustInt * 0.28 : dustInt, caveDustHud);
  }
}
