 ...
 */
const _lightKind: ('torch' | 'lava' | 'off')[] = new Array(MAX_LIGHTS).fill('off');
let _flickerTime = 0;
...
export function updateTorchLights(world, player, dt, maxLights = MAX_LIGHTS) {
  ...
  timer += dt;
  const needRescan = timer >= UPDATE_INTERVAL;
  if (needRescan) timer = 0;
  ...
  for (let i = 0; i < MAX_LIGHTS; i++) {
    if (_lightKind[i] === 'torch') {
      const phase = _flickerTime * 8.0 + i * 2.37;
      const flicker = 1.0 + 0.12 * Math.sin(phase) + 0.06 * Math.sin(phase * 2.73);
      pool[i].intensity = 1.2 * flicker;
    }
  }
}
