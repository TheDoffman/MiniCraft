import { SURFACE_Y_OFFSET } from './gameState';
...
/**
 * Deterministic [0,1) hash — used for terrain and per-block decisions.
 * Legacy call sites passed a 4th argument that JS ignored; keep that behavior for identical worlds.
 */
export function hashSeed(x: number, z: number, s: number, _legacyFourthIgnored?: number): number {
 ...
 */
export function surfaceHeight(wx, wz, seed, biomeOverride?: number) {
  const b = biomeOverride !== undefined ? biomeOverride : biomeParam(wx, wz, seed);
