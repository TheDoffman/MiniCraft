import { BLOCKS, BlockId, isDoorBottomId, isDoorTopId } from './blocktypes.js';
import { TorchAttach } from './torchAttach.js';
...
export class World {
  /**
   * @param {number} height Y extent (vertical column height)
   * @param {number} [seed]
   * @param {{ airOnly?: boolean }} [opts] If airOnly, chunks stay empty until set() (tests).
   */
  constructor(height, seed = 12345, opts = {}) {
    this.height = height;
    this.seed = seed;
    /** @type {Map<string, Uint8Array>} */
    this.chunks = new Map();
    /** Per-chunk flow distance 0–7 (0 = source). Only chunks that ever had water meta allocated. */
    /** @type {Map<string, Uint8Array>} */
    this._waterMeta = new Map();
    /** Per-block torch attachment (TorchAttach); unset = 0 = infer when rendering. */
    /** @type {Map<string, Uint8Array>} */
    this._torchMeta = new Map();
    /** Door bottom cells: facing (bits 0–1) + hingeRight (bit 2). */
    /** @type {Map<string, Uint8Array>} */
    ...
    for (let dx = -2; dx <= 2; dx++) {
      for (let dz = -2; dz <= 2; dz++) {
        for (let dy = 0; dy < 3; dy++) {
          ...
          /* Use getBlockIfLoaded so leaves that fall in an unloaded
             neighbouring chunk don't trigger a generation cascade.
             Those neighbours will fill in their own terrain when loaded. */
