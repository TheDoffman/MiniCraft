...
/** Inventory slot — meta = tool durability left */
export type InvSlot = { blockId: number; count: number; meta?: number };
...
export function canPickupBlock(blockId) {
  ...
  if (blockDef(blockId).fluid) return false;
...
/**
 * @returns {import('./inventory.ts').InvSlot[]}
 */
