...
import { ATLAS_TILES, TILE_PX } from './blocktypes';
...
function drawRawChicken(ctx, tx, ty) {
  const ox = tx * TILE_PX;
  const oy = ty * TILE_PX;
  const W = TILE_PX;
  ctx.clearRect(ox, oy, W, W);
...
function drawFeather(ctx, tx, ty) {
  const ox = tx * TILE_PX;
  const oy = ty * TILE_PX;
  const W = TILE_PX;
  ctx.clearRect(ox, oy, W, W);
...
function drawCookedPorkchop(ctx, tx, ty) {
  const ox = tx * TILE_PX;
  const oy = ty * TILE_PX;
  const W = TILE_PX;
  ctx.clearRect(ox, oy, W, W);
...
function drawCookedMutton(ctx, tx, ty) {
  const ox = tx * TILE_PX;
  const oy = ty * TILE_PX;
  const W = TILE_PX;
  ctx.clearRect(ox, oy, W, W);
...
function drawCookedChicken(ctx, tx, ty) {
  const ox = tx * TILE_PX;
  const oy = ty * TILE_PX;
  const W = TILE_PX;
  ctx.clearRect(ox, oy, W, W);
...
function drawDiamondPickaxe(ctx, tx, ty) {
  ctx.clearRect(tx * TILE_PX, ty * TILE_PX, TILE_PX, TILE_PX);
  drawPixelArt(ctx, tx, ty, {
    i: [55, 200, 205], I: [150, 255, 255], h: [88, 58, 36],
  }, [
    '................',
    '....iiIIIii.....',
    '....i.....i.....',
...
function drawDiamondAxe(ctx, tx, ty) {
  ctx.clearRect(tx * TILE_PX, ty * TILE_PX, TILE_PX, TILE_PX);
  drawPixelArt(ctx, tx, ty, {
    i: [55, 200, 205], I: [150, 255, 255], h: [88, 58, 36],
  }, [
...
function drawDiamondShovel(ctx, tx, ty) {
  ctx.clearRect(tx * TILE_PX, ty * TILE_PX, TILE_PX, TILE_PX);
  drawPixelArt(ctx, tx, ty, {
    i: [55, 200, 205], I: [150, 255, 255], h: [88, 58, 36],
  }, [
...
function drawDiamondSword(ctx, tx, ty) {
  ctx.clearRect(tx * TILE_PX, ty * TILE_PX, TILE_PX, TILE_PX);
  drawPixelArt(ctx, tx, ty, {
    i: [55, 200, 205], I: [150, 255, 255], h: [88, 58, 36],
  }, [
...
function drawCraftingTableTop(ctx, tx, ty) {
  drawPixelArt(ctx, tx, ty, {
    P: [188, 152, 98], p: [162, 130, 82], d: [140, 112, 70],
    G: [76, 76, 76], g: [56, 56, 56],
  }, [
    'PPPPPPPPPPPPPPPp',
    'PppppppppppppppP',
    'PpGGGGGGGGGGGgpP',
...
function drawCraftingTableSide(ctx, tx, ty) {
  drawPixelArt(ctx, tx, ty, {
    P: [188, 152, 98], p: [162, 130, 82], d: [140, 112, 70],
    D: [120, 96, 60], G: [76, 76, 76], g: [56, 56, 56],
    R: [140, 55, 40], B: [50, 65, 140],
  }, [
    'PPPPPPPPPPPPPPPp',
    'PpppppppppppppPp',
    'PpGGGGGGGGGGGgPp',
...
function drawCraftingTableBottom(ctx, tx, ty) {
  drawPixelArt(ctx, tx, ty, {
    P: [162, 130, 82], p: [140, 112, 70], d: [120, 96, 60],
  }, [
    'PPPPPPPPPPPPPPPp',
    'PppppppppppppppP',
    'PppppppppppppppP',
...
function drawSteak(ctx, tx, ty) {
  drawPixelArt(ctx, tx, ty, {
    B: [120, 60, 35], b: [95, 45, 25],
    M: [155, 85, 55], m: [135, 70, 42],
    F: [200, 180, 140], f: [175, 155, 115],
  }, [
    '................',
    '................',
    '................',
...
function drawWheatItem(ctx: CanvasRenderingContext2D, tx: number, ty: number) {
  const x0 = tx * TILE_PX, y0 = ty * TILE_PX;
  const stalk = '#c8a83e';
  const grain = '#e8c840';
  const dark = '#8a6e20';
  for (let i = 6; i <= 9; i++) {
    for (let j = 4; j <= 14; j++) {
      ctx.fillStyle = (j % 2 === 0) ? stalk : dark;
      ctx.fillRect(x0 + i * CELL, y0 + j * CELL, CELL, CELL);
    }
  }
  for (const [gx, gy] of [[5,3],[6,2],[7,2],[8,2],[9,2],[10,3],[5,5],[10,5]]) {
    ctx.fillStyle = grain;
    ctx.fillRect(x0 + gx * CELL, y0 + gy * CELL, CELL, CELL);
  }
}

function drawBread(ctx: CanvasRenderingContext2D, tx: number, ty: number) {
  const x0 = tx * TILE_PX, y0 = ty * TILE_PX;
  const crust = '#c8872e';
  const inner = '#e8b858';
  const top = '#d4993a';
  for (let i = 3; i <= 12; i++) {
    for (let j = 6; j <= 11; j++) {
      const edge = (j === 6 || j === 11 || i === 3 || i === 12);
      ctx.fillStyle = edge ? crust : (j === 7 ? top : inner);
      ctx.fillRect(x0 + i * CELL, y0 + j * CELL, CELL, CELL);
    }
  }
}

function drawEgg(ctx: CanvasRenderingContext2D, tx: number, ty: number) {
  const x0 = tx * TILE_PX, y0 = ty * TILE_PX;
  const shell = '#f0e8d0';
  const shadow = '#d0c8a8';
  ...
  for (const [px, py] of pixels) {
    const isHighlight = px <= 7 && py <= 6;
    const isShadow = px >= 9 || py >= 10;
    ctx.fillStyle = isHighlight ? highlight : (isShadow ? shadow : shell);
    ctx.fillRect(x0 + px * CELL, y0 + py * CELL, CELL, CELL);
  }
}

function drawDiamondHelmet(ctx: CanvasRenderingContext2D, tx: number, ty: number) {
  const x0 = tx * TILE_PX, y0 = ty * TILE_PX;
  const dc = '#5ce0d0';
  const dd = '#3aaea0';
  for (let i = 3; i <= 12; i++) {
    for (let j = 4; j <= 11; j++) {
      if (j >= 9 && i >= 5 && i <= 10) continue;
      const edge = (j === 4 || i === 3 || i === 12);
      ctx.fillStyle = edge ? dd : dc;
      ctx.fillRect(x0 + i * CELL, y0 + j * CELL, CELL, CELL);
    }
  }
}

function drawDiamondChestplate(ctx: CanvasRenderingContext2D, tx: number, ty: number) {
  const x0 = tx * TILE_PX, y0 = ty * TILE_PX;
  const dc = '#5ce0d0';
  const dd = '#3aaea0';
  for (let i = 3; i <= 12; i++) {
    for (let j = 2; j <= 13; j++) {
      if (j >= 5 && j <= 13 && i >= 6 && i <= 9) continue;
      const edge = (j === 2 || j === 13 || i === 3 || i === 12);
      ctx.fillStyle = edge ? dd : dc;
      ctx.fillRect(x0 + i * CELL, y0 + j * CELL, CELL, CELL);
    }
  }
...
function drawDiamondLeggings(ctx: CanvasRenderingContext2D, tx: number, ty: number) {
  const x0 = tx * TILE_PX, y0 = ty * TILE_PX;
  const dc = '#5ce0d0';
  const dd = '#3aaea0';
  for (let i = 3; i <= 12; i++) {
    for (let j = 3; j <= 13; j++) {
      if (j >= 6 && i >= 7 && i <= 8) continue;
      const edge = (j === 3 || j === 13 || i === 3 || i === 12);
      ctx.fillStyle = edge ? dd : dc;
      ctx.fillRect(x0 + i * CELL, y0 + j * CELL, CELL, CELL);
    }
  }
...
function drawDiamondBoots(ctx: CanvasRenderingContext2D, tx: number, ty: number) {
  const x0 = tx * TILE_PX, y0 = ty * TILE_PX;
  const dc = '#5ce0d0';
  const dd = '#3aaea0';
  for (let i = 4; i <= 6; i++) {
    for (let j = 6; j <= 12; j++) {
      ctx.fillStyle = (j === 6 || i === 4) ? dd : dc;
      ctx.fillRect(x0 + i * CELL, y0 + j * CELL, CELL, CELL);
    }
  }
  for (let i = 9; i <= 11; i++) {
    for (let j = 6; j <= 12; j++) {
      ctx.fillStyle = (j === 6 || i === 11) ? dd : dc;
      ctx.fillRect(x0 + i * CELL, y0 + j * CELL, CELL, CELL);
    }
  }
}
...
export function buildAtlasCanvas() {
  ...
  drawCraftingTableTop(ctx, 10, 3);
  drawCraftingTableSide(ctx, 11, 3);
  drawCraftingTableBottom(ctx, 12, 3);
  drawLava(ctx, 13, 3);

  drawMutton(ctx, 0, 4);
  drawWool(ctx, 1, 4);
  drawRawChicken(ctx, 2, 4);
  drawFeather(ctx, 3, 4);
  drawCookedPorkchop(ctx, 4, 4);
  drawCookedMutton(ctx, 5, 4);
  drawCookedChicken(ctx, 6, 4);
  drawSteak(ctx, 7, 4);
  drawInkSac(ctx, 8, 4);
  drawDiamondPickaxe(ctx, 0, 5);
  drawDiamondAxe(ctx, 1, 5);
  drawDiamondShovel(ctx, 2, 5);
  drawDiamondSword(ctx, 3, 5);
  drawWheatItem(ctx, 4, 5);
  drawBread(ctx, 5, 5);
  drawEgg(ctx, 6, 5);
  drawDiamondHelmet(ctx, 7, 5);
  drawDiamondChestplate(ctx, 8, 5);
  drawDiamondLeggings(ctx, 9, 5);
  drawDiamondBoots(ctx, 10, 5);
