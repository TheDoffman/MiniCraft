...
import { buildPlayerModelMaterials } from './playerTextures';
...
export function createPlayerModel(atlasTexture) {
  ...
  const nameTag = createNameTagSprite('Steve');
  const nameTagY = shoulderY + headS + 0.22;
  nameTag.position.set(0, nameTagY, 0);
...
function createNameTagSprite(name: string): THREE.Sprite {
  const canvas = document.createElement('canvas');
  const ctx = canvas.getContext('2d')!;
  const fontSize = 48;
  const font = `bold ${fontSize}px "Press Start 2P", monospace`;
  ctx.font = font;
  ...
  applyNameTagWorldScale(sprite, w, h);
  sprite.renderOrder = 10;
  (sprite as any)._nameTagCanvas = canvas;
  (sprite as any)._nameTagTex = tex;
  return sprite;
}

export function setPlayerNameTag(group: THREE.Group, name: string) {
  const tag = group.userData.nameTag as THREE.Sprite | undefined;
  if (!tag) return;
  const canvas = (tag as any)._nameTagCanvas as HTMLCanvasElement;
  const tex = (tag as any)._nameTagTex as THREE.CanvasTexture;
  const ctx = canvas.getContext('2d')!;
  const fontSize = 48;
  const font = `bold ${fontSize}px "Press Start 2P", monospace`;
  ctx.font = font;
  ...
  applyNameTagWorldScale(tag, w, h);
}

/** World-space size: short names stay readable; long names cap width so the tag does not span the screen. */
function applyNameTagWorldScale(sprite: THREE.Sprite, canvasW: number, canvasH: number) {
  const targetHeight = 0.09;
  const maxWidth = 0.42;
  const aspect = canvasW / canvasH;
  let sx = targetHeight * aspect;
  let sy = targetHeight;
  if (sx > maxWidth) {
    const f = maxWidth / sx;
    sx = maxWidth;
