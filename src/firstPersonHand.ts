...
import { BLOCKS, TILE_PX, ATLAS_TILES, TOOL_INFO, blockDef } from './blocktypes';
import { buildFirstPersonArmMaterialSet } from './playerTextures';
...
/** `fpArmRig.rotation.x = PI/2` — convert a point from hand-group space to armRig-local. */
function groupPointToArmRigLocal(px: number, py: number, pz: number) {
  return { x: px, y: pz, z: -py };
}

const _qInvArmRig = new THREE.Quaternion();
const _qHeldGroupEuler = new THREE.Quaternion();
const _eHeldTmp = new THREE.Euler(0, 0, 0, 'YXZ');
const _axX = new THREE.Vector3(1, 0, 0);

/** Match old “held as child of fpHand group” orientation when parented under `fpArmRig`. */
function setHeldLocalQuatFromGroupEuler(mesh: THREE.Mesh, rx: number, ry: number, rz: number, side: number) {
  _qInvArmRig.setFromAxisAngle(_axX, -Math.PI / 2);
  _eHeldTmp.set(rx, ry * side, rz * side, 'YXZ');
  _qHeldGroupEuler.setFromEuler(_eHeldTmp);
  mesh.quaternion.copy(_qInvArmRig).multiply(_qHeldGroupEuler);
}
...
export function createFirstPersonHand(atlasTexture) {
  ...
  /** Draw after the hand voxels so the item sits visibly on the palm. */
  heldMesh.renderOrder = FP_RO + 2;
  heldMesh.visible = false;

  /* Parent to armRig (Group), not fpHandStrip (Mesh) — Mesh children can fail to draw in some cases. */
  armRig.add(heldMesh);
  group.add(armRig);
  ...
  group.userData.fpArmH = armH;
  group.userData.fpHandStripH = handStripH;
  group.userData.fpArmW = armW;
  group.userData.fpArmD = armD;
  /** @deprecated Kept for any external reads; held item is parented to fpHandStrip now. */
...
export function updateFirstPersonHeldBlock(hand, blockId, leftHanded) {
  ...
  const ps = PLAYER_H / 32;
  const armH = typeof hand.userData.fpArmH === 'number' ? hand.userData.fpArmH : 12 * ps;
  const armW = typeof hand.userData.fpArmW === 'number' ? hand.userData.fpArmW : 4 * ps;
  const armD = typeof hand.userData.fpArmD === 'number' ? hand.userData.fpArmD : 4 * ps;
  const armHalfW = typeof hand.userData.armHalfW === 'number' ? hand.userData.armHalfW : armW * 0.5;
  /** Negative X = opposite lateral side of the arm (pinky side vs thumb); armRig.scale.x still mirrors for left hand. */
  const ax = -(armHalfW + 0.04);
  const palmZ = typeof hand.userData.palmLocalY === 'number' ? hand.userData.palmLocalY : -armH - 1.5 * ps;

  /**
   * Targets in fpHand *group* space (camera-aligned when synced): same convention as the pre-handStrip
   * parenting layout. Convert to fpArmRig-local via inverse of armRig.rotation.x = PI/2.
   * Higher `gy` lifts the block toward the top of the hand in view.
   */
  const applyGroupTarget = (gx: number, gy: number, gz: number) => {
    const l = groupPointToArmRigLocal(gx, gy, gz);
    mesh.position.set(l.x, l.y, l.z);
  };

  const gyBlock = 0.085;
  applyGroupTarget(ax, gyBlock, palmZ + 0.072);
  setHeldLocalQuatFromGroupEuler(mesh, -0.1, -0.22, -0.04, side);
  ...
  if (isTool) {
    applyGroupTarget(ax, 0.02, palmZ + 0.078);
    setHeldLocalQuatFromGroupEuler(mesh, -0.55, -0.42, 0.12, side);
    mesh.scale.setScalar(1);
  } else if (isItem) {
    applyGroupTarget(ax, 0.04, palmZ + 0.09);
    setHeldLocalQuatFromGroupEuler(mesh, -0.3, -0.35, 0.75, side);
