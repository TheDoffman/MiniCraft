...
import { sampleWeather, smoothWeatherSample } from '../src/weather';
import { biomeParam, getBiome } from '../src/worldgen';
...
describe('weather', () => {
  ...
  it('smoothWeatherSample eases toward target', () => {
    ...
    const prevStrength = cur.strength;
    const next = smoothWeatherSample(cur, target, 0.1, 5);
    expect(next).toBe(cur);
    expect(next.strength).toBeGreaterThan(prevStrength);
