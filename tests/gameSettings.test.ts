...
import { loadSettings, saveSettings, DEFAULT_GAME_SETTINGS } from '../src/gameSettings';
...
describe('gameSettings', () => {
  ...
  it('defaults include extended fields', () => {
    ...
    expect(DEFAULT_GAME_SETTINGS.showMinimap).toBe(true);
    expect(DEFAULT_GAME_SETTINGS.ambientMusicBed).toBe(true);
    expect(DEFAULT_GAME_SETTINGS.weatherEffectsQuality).toBe('balanced');
  ...
  it('loadSettings falls back invalid weather effects quality to default', () => {
    localStorage.setItem(
      'minicraft-settings-v1',
      JSON.stringify({ weatherEffectsQuality: 'ultra' }),
    );
    expect(loadSettings().weatherEffectsQuality).toBe('balanced');
  });
